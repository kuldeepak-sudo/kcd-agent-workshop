...
| Part 1's allowlist/rate limit stop working again after running `part2-token-exchange/setup.sh` | The Tyk OSS Gateway's registered-API store is an `emptyDir` — Part 2's `helm upgrade`/pod restart wipes it, and Part 2's own script re-registers Part 1's `mock-mcp-governed` as part of cleaning up after itself | `part2-token-exchange/setup.sh` already re-registers it via `/tyk/mcps` (a real bug, fixed 2026-09-01: it used to use `/tyk/apis/oas`, which put Part 1 back in an ungoverned state every time). If `curl /tyk/mcps` still comes back `[]` after running Part 2's setup, re-run `../setup.sh` |
